...
There's nothing there if it's surrounded by < and > <secretsssss>

+ to create a bullet 
- to make another bullet
* another way
BUT gotta add another return or it'll just be on one line

* code output and text in one doc
- knit to useful formats like html or pdf
- git friendly / version control
- reproducible 
- updating capabilities
- your code is legible
- can decide what kind of text we can display (like the reader may not need to know what the code looks like, but we could show them the data viz/output/conclusions, but not all the code)
- flexible formatting
- simple syntax and auto-referencing
...
there needs to be something on every line to make it look like a table:

Raw datasets                      | Processed 
-------------                     | -------------
ECOTOX_Neonicotinoids_Mortality   | Neonic
NTL-LTER_Lake_Chemistry           | NTL-LTER_Lake_Chemistry_Nutrients
EPAair_O3_NC2017                  | 
EPAair_PM25_NC2018                | EPA_NCAir_2017_18_Processed
...
```{r hidden installation, include=FALSE}
...
```{r hidden read csv, include=FALSE}
...
```{r tn_ug mean min max}
Lake_sum <- Lake_gather %>%
  group_by() %>%
...
Create a fourth and fifth R chunk that generates two plots (one in each chunk): one for total N over time with different colors for each lake, and one with the same setup but for total P. Decide which geom option will be appropriate for your purpose, and select a color palette that is visually pleasing and accessible. Customize the chunk options such that the final figures are displayed but not the code used to generate the figures. < delete : In addition, customize the chunk options such that the figures are aligned on the left side of the page.> Lastly, add a fig.cap chunk option to add a caption (title) to your plot that will display underneath the figure.
