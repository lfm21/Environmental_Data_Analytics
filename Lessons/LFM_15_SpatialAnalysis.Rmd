...
>elevation or rainfall (these are often rasters, but contours can also rep elev)
...
EPAair_PM_avg <- EPAair_PM25_NC2018_raw %>% #24 obs, 6 var
...
ggplot(EPAair_PM_avg, aes(x=SITE_LONGITUDE, y=SITE_LATITUDE)) + 
  geom_point(aes(color=meanPM), size=4) + 
  coord_equal() 
```
...
myMap  <- leaflet(data = EPAair_PM_avg) %>% 
  addTiles() %>% #tiles is the background
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius = (~meanPM*2), #scale w/ mean PM values
                   stroke = FALSE, 
                   fillOpacity = 0.3, 
                   popup = ~as.character(`Site.Name`)) #can specify what pops up

#Show the map
myMap 
#can save as webpage by hitting Publish, and it'll go to our git page but have to install some pkgs
```
...
EPAair_PM_avg_sf <- st_as_sf(EPAair_PM_avg,coords =  c('SITE_LONGITUDE','SITE_LATITUDE'),crs=4326) #converts df to spatial feature df

#Do you see a new column name?                
colnames(EPAair_PM_avg_sf) #added geomoetry col (changed 6 col to 5 col)
...
ggplot() + 
  geom_sf(data=EPAair_PM_avg_sf, aes(color=meanPM), size=4) #geom_sf plots as a spatial feat (if there's an error in grid.Call, change size of plot window)
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1) #this is 0.1 decimal degrees (arc_degrees)
...
* What is the extent of this projection? proj4 used in R a lot
...
EPAair_PM_avg_sf_UTM <- st_transform(EPAair_PM_avg_sf, crs=26917) #takes the exisitng spatial df and converts to existing coord sys : UTM Zone 16N
...
ggplot() + 
  geom_sf(data=EPAair_PM_avg_sf,aes(size=Dist2DA)) #then you can see if say, distance is a factor 
```
...
#Coordinate Reference System:
  #EPSG: 4269 
  #proj4string: "+proj=longlat +datum=NAD83 +no_defs" (the long way of saying the above)

#Column names
names(counties_sf) #the geometry is a list of list of coord pairs

#Reveal the number of features in this dataset
nrow(counties_sf)

#Reveal the extent of this dataset via the st_bbox() function
st_bbox(counties_sf) #boundary box
...
#Check the CRS
st_crs(huc8_sf)
#there's not an EPSG code for ALL the coord sys, but it has the raw definitions so the engines can find it (it uses Albers equal area)
...
mapview(huc8_sf)
plot(huc8_sf['SUB'],
     graticule = st_crs(4326), #a geographic grid, you have to give it a crs
     axes = TRUE)
...
mapview(upperNeuse_sf) #geojson is text based web app
...
state_sf <- counties_sf %>% 
  #group_by(STATEFP) %>% unnecessary line
...
state_sf2 <- st_union(counties_sf) #geometric operation that unions all the results
mapview(state_sf2)
```
...
huc2_sf <- huc8_sf %>%
  group_by(SUB) %>%
  summarize(area = sum(AREA)) #have to groupby before summarizing
  
mapview(huc2_sf)
mapview(huc2_sf['AREA'])

#4326 is MERCATOR, leaflet needs it
```
...
mapview(huc2_utm) #WE SEE THAT boundaries fall outside of NC, LET'S CLIP IT w/ intersection fun
...
huc0302_counties <- huc2_utm %>%
  filter(SUB == '0302')
huc0302_counties <- st_intersection(counties_sf_utm, huc2_utm)
...
demog_df <- read.csv('./Data/Spatial/NC_Demography.csv') %>% 
  mutate(FIPS = as.factor(FIPS))                                 #Convert the FIPS variable from number to a factor

#Join the two datasets using "GEOID" from the left dataset and "FIPS" from the right
counties_sf_utm_join <- counties_sf_utm %>% 
  left_join(y = demog_df,by = c("GEOID" =  "FIPS")) #labeling the columns in the dataset that we're joining: it's called FIPS in one set and GEOID in another, making them a single column
#specifying left table, means joining features
...
#to tell tablular df to be a spatial df, use st_to_sf
...
triCo_centroids <-  st_centroid(triCo) #overwriting the values of centroids into the polygon
...
triCo_in2km <- st_buffer(triCo, -2000) #-ve number can shrink the feature
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>% #combining all centroids into 1 row(1 geometry)
  st_convex_hull()
...
theSite_sfp <- st_point(c(userLng,userLat)) #spatial feature point (so that the point refers to an actual place)

#Create a simple features column from the point geometry object
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) #now it knows its crs

#Transform the mask to match the CRS of the counties dataset
theSite_sfc_transformed <- st_transform(theSite_sfc, crs = st_crs(counties_sf_utm)) #transforming to utm

#Create a boolean mask  to see what county it belings in
resultMask <- st_intersects(counties_sf_utm, #intersects see what feature the point intersects
                            theSite_sfc_transformed,
                            sparse = FALSE) #The `sparse` option returns a Boolean mask
view(resultMask) #returns one TRUE (all others are false)
...
counties_dist <- st_buffer(counties_sf_utm, 30)
