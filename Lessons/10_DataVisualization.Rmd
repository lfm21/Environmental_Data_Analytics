...
O3plot <- ggplot(EPAair) + #ggpplot theme changes w/in 1 line, don;t have to redefine a bunch for each graph
  geom_point(aes(x = Date, y = Ozone)) 
print(O3plot) #default

O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #add theme as if its a layer, this one is default
print(O3plot1)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #has box and white bkgrd
print(O3plot2)

O3plot3 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_light()
print(O3plot3) #all gray lines

O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()
print(O3plot4) #no grid, just black line for axes
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #base font is 11 or 12
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top") #alternative: legend.position + legend.justification; can tell it to put legend in particular coord in graph 
...
O3plot5 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme #add as a layer
print(O3plot5)  

theme_set(mytheme) #if you're making a lot of plots for your whole session, can be overridden by specifiying w/in a single ggplot
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  #geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), #made background 0-50 green
            #ymin = 0, ymax = 50, fill = "green") +
  #geom_rect(xmin = as.Date("2016-01-01"), xmax = as.Date("2020-01-01"), 
            #ymin = 50, ymax = 100, fill = "yellow") + #makes 51-100 yellow
  geom_point() + #type of plot
  geom_text(x = as.Date("2019-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #labeled good at y=45 BUT you have to tell it to look for a date
  geom_text(x = as.Date("2019-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") +
  scale_x_date(limits = as.Date(c("2017-01-01", "2018-12-31")), #told it only to use month names as label instead of number
    date_breaks = "2 months", date_labels = "%b %y") + #broke up dates into every other month
  ylab(expression("O"[3]* " AQI Value")) + #brackets give you subscript for O3, superscript is a carat
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #angles the text; hjust moves text horizontally; vjust for vertical
print(O3plot7)  
...
library(viridis) #ff is last digits on hexcode
library(RColorBrewer) #diverging to make colorblind safe
library(colormap)
...
NvsP2 <-
  ...
  geom_point(alpha = 0.8, size = 3,color = "magenta") +
xlab(expression("total phosphorus (\U003BCg/L)")) +
ylab(expression("total nitrogen (\U003BCg/L)")) +
#labs(x = "Total P", y = "Total N", fill = "Depth, m", shape = lakename)
  # change your legend labels here
  guides(fill=guide_legend(title="depth")) +
  scale_shape_manual(values = c(15, 17)) + #these are shape numbers; for other shapes you can define fill and outline
...
ylab() +
 ...
 #scale_fill_distiller(palette = “Blues”, direction = 1) + # use scale_color_brewer for discrete variables
