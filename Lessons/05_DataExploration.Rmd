...
```{r}
# Absolute file path (not recommended, because...? you want other users to be able to get to it..?)
...
> ANSWER: 
naming conventions, assembler, type, what it is, etc.
...
# Rename columns (gotta figure out what to rename )
...
str(USGS.flow.data) #structure (like integer or numerical)
dim(USGS.flow.data) #dimensions, that there's 33,216 entries, 40 pages, etc
length(USGS.flow.data) #

head(USGS.flow.data) 
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5) #last five columns
USGS.flow.data[30000:30005, c(3, 8, 14)] #the range

class(USGS.flow.data$datetime)
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data) #basic stats about the data
summary(USGS.flow.data$discharge.mean) #numerical
summary(USGS.flow.data$discharge.mean.approval) # a factor
