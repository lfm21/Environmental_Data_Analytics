...
# Format sampledate as date (when R uploads/ each time you re-import, we need to reformat the date using as.Date again)
NTL.phys.data.PeterPaul$sampledate <- as.Date(NTL.phys.data.PeterPaul$sampledate, format = "%m/%d/%y")

# Select Peter and Paul Lakes from the nutrient dataset, (filter chooses rows, select chooses columns)
...
summary(NTL.nutrient.data.PeterPaul$comments) #numeric will store nothing as NA, but this is a factor so it stores as just blank
#use function drop.levels on whole dataset it'll tell us which levels exist in the dataset
...
# Save processed nutrient file (row.names = FALSE so it won't save row names)
...
#(can put a -minus sign in front of column name to remove column from dataset)
...
#gathered data can make it easier to make a graph or something
...
# Split components of cells into multiple columns, but R will eliminate the original column; then the class will be a character (it can't be date unless its a yyyy-mm-dd)
...
```{r}
#mutating joins (more columns, more inclusive) vs filtering joins (fewer joins, exclusive)
#remember that the phys.data had more sampling dates than nutrient.data: so phys will be x and nutrient will be y
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.skinny, NTL.nutrient.data.PeterPaul.skinny)
#coerced lakename from factor to character
View(NTL.phys.nutrient.data.PeterPaul)

write.csv(NTL.phys.nutrient.data.PeterPaul, row.names = FALSE, file ="./Data/Processed/NTL-LTER_Lake_Nutrient_Phys_PeterPaul_Processed.csv")
```
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>%
  group_by(lakename) %>%
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #only removes situations where all three are NA
  summarise(meantemp = mean(temperature_C), #the group_by fun will give us both Peter and Paul, bc that's waht's in this dataset under lakename
...
write.csv(NTL.phys.nutrient.data.PeterPaul, row.names = FALSE, file = "./Data/Processed/NTL-LTER_Lake_Summaries_PeterPaul_Processed.csv")
