...
head(NTL.phys.data) #just gives fiorst 6 rows unless otherwise specified
...
dim(NTL.phys.data.surface1) #storing as new row numbers than in original dataframe
...
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) #comma means 'I'm gonna gove another conditional statement now'
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #columns everything including sampledate thru temperature; gives us same amount of rows but only those columns
...
#this file will automatically overwrite, so be careful; comment out the line of code !
```
...
How did data wrangling help us to generate a processed dataset? How does this impact our ability to analyze and answer questions about our data?
 > It totally helps. We can organize the data in the way that we want to, and also keep the Environment cleaner if we want.
