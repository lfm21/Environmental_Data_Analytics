...
* LAGOSNE: Multiscaled geospatial and temporal data for U.S. lakes 
  *for hydro data class next semester
...
* API (retrieve data from standard code; data must be in prescribed format) 
  - mgrast is metagenomics research
...
url <- "https://en.wikipedia.org/wiki/List_of_rivers_by_length"  #providing a path to this url

# Reading the HTML code from the website
webpage <- read_html(url)
#OR we could read_html("whateverwebsite.com")
...
riverdata <- data_frame(Name = Name[6:188], 
                        ...
                        Outflow = Outflow)
#problems: commas in numbers, asterisks, blank cells, brackets, parentheses

# Remove unnecessary text from within cells
riverdata$Name <- str_replace(riverdata$Name, "\\[.*\\]", "") #\\ means remove all spaces before brackets, and all things in brackets; then all citations and things in brackets are now gone
riverdata$Name <- str_replace(riverdata$Name, "\n", "") #removed \n from Name column and 
riverdata$Name #just regular names
...
class(riverdata$Length.km) #it's all still character

# Turn numeric values from character to numeric
riverdata$Length.km <- as.numeric( sub(",", "", riverdata$Length.km)) #replacing the comma with a nothing
riverdata$DrainageArea.km2 <- as.numeric( sub(",", "", riverdata$DrainageArea.km2)) #Warning message: NAs introduced by coercion
riverdata$Discharge.m3s <- as.numeric( sub(",", "", riverdata$Discharge.m3s))
  
# Plot the relationships just to see
ggplot(riverdata, aes(x = Length.km, y = Discharge.m3s, fill = DrainageArea.km2)) + 
  ...
  geom_label(data = subset(riverdata, Name == "Flinders River" | Name == "Amazon–Ucayali–Tambo–Ene–Mantaro"), 
            aes(label = Name), 
            alpha = 0.8, nudge_x = 100, nudge_y = 0.15)
