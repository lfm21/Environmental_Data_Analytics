...
#past0 takes the 2 statements in the function to put them together
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
#tell it to appky the function we made above to that column
...
> We would want to keep NAs in qualitative data, to know what we don't know (it's also an answer sometimes); and in some datasets NA could translate as undetectable.

```{r}
#creating new dataframe for flow data complete, by removing the entire row if there's not observ for every column
USGS.flow.data.complete <- na.omit(USGS.flow.data)
#complete.cases can show which rows are complete
dim(USGS.flow.data)
...
#shows the amount of samples at certain values ; the draw-quantiles shows lines at eah specification
ggplot(USGS.flow.data.complete) +
